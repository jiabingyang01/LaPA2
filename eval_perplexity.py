...
def cal_ppl(text=None, args=None):
    # Use bos_token for LLaMA-like models, eos_token for GPT2-like models
    prefix_token = tokenizer.bos_token if tokenizer.bos_token else tokenizer.eos_token
    input_text = torch.tensor([tokenizer(prefix_token + text).input_ids]).to(args.device)
