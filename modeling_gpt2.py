...
# ============ LaPA2 Boost Config ============
# Length-Aware Logarithmic Scaling on prefix attention logits:
#   attn_logit[:, :, :, :boost_len] += alpha * log(kv_seq_len / boost_len)
# Set globally before each forward pass via set_boost_config(...)
_boost_config = {
    "enabled": False,
    "alpha": 0.5,
    "boost_len": 0,
...
def set_boost_config(enabled=False, alpha=0.5, boost_len=0):
    global _boost_config
    _boost_config["enabled"] = enabled
    _boost_config["alpha"] = alpha
    _boost_config["boost_len"] = boost_len
# ============ End LaPA2 Boost Config ============
...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, use_soft_prefix=False, output_attentions=False, base_decoding=False, prompt_len=None, hard_prefix_len=None, task_mode=None):
        w = torch.matmul(q, k)
        if self.scale:
        ...
        # ============ LaPA2 Boost ============
        # Length-Aware Logarithmic Scaling: add alpha * log(kv_len / boost_len)
        # to the prefix-region attention logits at every decoding step (q_len == 1).
        # During the prompt forward pass (q_len > 1) we leave logits untouched.
        if _boost_config["enabled"] and w.size(-2) == 1:
            boost_len = _boost_config["boost_len"]
            kv_seq_len = w.size(-1)
            if boost_len > 0 and boost_len < kv_seq_len:
                alpha = _boost_config["alpha"]
                w[:, :, :, :boost_len] = w[:, :, :, :boost_len] + alpha * math.log(kv_seq_len / boost_len)
        # ============ End LaPA2 Boost ============
    ...
    def forward(
        ...
        use_soft_prefix=False,
        prefix_id=None,
        base_decoding=False,
        prompt_len=None,
        hard_prefix_len=None,
        task_mode=None,
    ):
        ...
        attn_outputs = self._attn(query, key, value, attention_mask, head_mask, use_soft_prefix, output_attentions, base_decoding, prompt_len, hard_prefix_len, task_mode)
        a = attn_outputs[0]
...
class Block(nn.Module):
    ...
    def forward(
        ...
        attn_outputs = self.attn(
            ...
            use_soft_prefix=use_soft_prefix,
            prefix_id=prefix_id,
            base_decoding=base_decoding,
            prompt_len=prompt_len,
            hard_prefix_len=hard_prefix_len,
            task_mode=task_mode,
        )
